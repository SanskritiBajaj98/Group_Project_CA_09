...
total_litres_of_pure_alcohol - Decimal 
...
```{r beer_plot}
top_25_beer_consuming <-  drinks %>% 
  arrange(desc(beer_servings)) %>%
  head(25)
  
ggplot(top_25_beer_consuming, aes(x=beer_servings, y=fct_reorder(country, beer_servings))) +
  ggtitle ('Top 25 beer serving countries') +
...
```{r wine_plot}
top_25_wine_consuming <-  drinks %>% 
  arrange(desc(wine_servings)) %>%
  head(25)
  
ggplot(top_25_wine_consuming, aes(x=wine_servings, y=fct_reorder(country, wine_servings))) +
  ggtitle ('Top 25 wine serving countries') +
  labs(title = 'Top 25 wine consuming countries', x = 'Wine Serving', y = 'Country') +
  geom_col()
...
top_25_spirit_consuming <-  drinks %>% 
  arrange(desc(spirit_servings)) %>%
  head(25)
  
ggplot(top_25_spirit_consuming, aes(x=spirit_servings, y=fct_reorder(country, spirit_servings))) +
  ggtitle ('Top 25 spirit serving countries') +
  labs(title = 'Top 25 spirit consuming countries', x = 'Spirit Serving', y = 'Country') +
  geom_col()
...
- Are there any missing values (NAs)? Are all entries distinct or are there duplicate entries?
  There are no missing values. There are duplicate titles, as there are 54 non-unique rows. 
```{r, missing_value}
   which(is.na(movies))
...
summary_ratings <- movies %>%
  group_by(genre) %>%
  summarize(average_ratings = mean(rating), median_ratings=median(rating),sd_ratings=StdDev(rating))%>%
  arrange(genre)
summary_ratings
