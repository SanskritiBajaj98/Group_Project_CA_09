...
tidy_GDP_data  <- gather(UN_GDP_data, Year, GDP_components, "1970":"2017", factor_key=TRUE)%>%
                  ...
                  mutate(IndicatorName = if_else(IndicatorName == 'Gross Domestic Product (GDP)', 'GDP', IndicatorName))%>%
...
```{r}
Country_for_plots_2 = c("Germany", "India", "United States") #Listing the specific categories for which data is required 

GDP_components_for_plots_2 = c("Household expenditure","Government expenditure"	,"Gross capital formation","Exports","Imports","GDP")

tidy_GDP_subset_data_2 <- subset(tidy_GDP_data, Country %in% Country_for_plots_2 & IndicatorName %in% GDP_components_for_plots_2)


new_table <- pivot_wider(tidy_GDP_subset_data_2, names_from = IndicatorName ,values_from = GDP_components)
new_table$Net_exports <- new_table$Exports - new_table$Imports
new_table$Calculated_GDP <- new_table$`Household expenditure` + new_table$`Government expenditure`+ new_table$`Gross capital formation`+ new_table$Net_exports
new_table$Percentage_deviation_in_GDP <-(((new_table$Calculated_GDP/new_table$GDP) - 1)*100)
new_table$Household_expenditure_proportion <-(((new_table$`Household expenditure`/new_table$GDP)))
new_table$Government_expenditure_proportion <-(((new_table$`Government expenditure`/new_table$GDP)))
new_table$Gross_capital_proportion <-(((new_table$`Gross capital formation`/new_table$GDP)))
new_table$Net_exports_proportion <-(((new_table$Net_exports/new_table$GDP)))
new_table
```


```{r,  out.width="100%"}
new_table_2 <- pivot_longer(new_table, cols = 13:16, names_to = "IndicatorNames", values_to = "GDP_components")
new_table_2
ggplot(new_table_2, aes(x=Year, y = GDP_components, group = IndicatorNames, color = IndicatorNames)) +  
  facet_wrap(~Country)+
  geom_line()+
  labs (title = "GDP components over time", x="Year", y="proportion") + 
  scale_x_discrete(breaks=seq(1970,2017,by=10))+ 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  theme_bw() +
  scale_y_continuous(labels= scales::percent_format(accuracy = 0.1),limits=c(0,0.6))+
  NULL
