...
- Are there any missing values (NAs)? Are all entries distinct or are there duplicate entries? No missing value. There are duplicates, as there are 54 non-unique rows.
```{r}
...
```{r}
movies %>% #selecting the movies data
  group_by(director) %>% #grouping the data by director 
  ...
  arrange(desc(average_gross))%>% #arranging the data in descending order 
  head(15) #returning the top 15 values 
...
Higher standard deviation of return conveys the higher risk assumed for achieving expected returns. The higher the risk assumed (higher standard deviation) the higher are the returns (higher expected monthly returns).
...
```
```{r}
glimpse(vaccinations) #summarizing vaccinations dataset

```
```{r}

pct_vaccinations <- vaccinations %>% #filtering percentage of population vaccinated 
...
```{r}
glimpse(election2020_results) #summarizing election data 
```
```{r}

pct_votes <- election2020_results %>% #filtering population as per the votes given to Republican party 
  group_by(fips) %>%
...
```{r}
#filtering the population that has voted Republican and taken the vaccine 
...
- Approximately how much time did you spend on this problem set:  hours 
- What, if anything, gave you the most trouble: Recreating charts 
