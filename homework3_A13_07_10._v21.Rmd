...
ggplot(yield_diff_data, aes(x = date, y = yield_difference))+
  ...
  geom_rect(data=filter(recessions, from >= "1960-01-01"), inherit.aes = F, mapping=aes(xmin=From, xmax= To, ymin=-Inf, ymax=Inf), color="black", alpha=0.5) 
  NULL
...
```{r}
GDP_components_for_calculation = c("Household expenditure","Government expenditure"	,"Gross capital formation","Exports","Imports")

tidy_GDP_subset_data_for_GDP <- subset(tidy_GDP_data, IndicatorName %in% GDP_components_for_calculation)
tidy_GDP_subset_data_for_GDP
tidy_GDP_subset_data_for_GDP <- tidy_GDP_subset_data_for_GDP %>% 
